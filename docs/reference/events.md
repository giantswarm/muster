...
Muster generates events for all major operations involving:
...
These are the two resource types accepted by `--resource-type` (`mcpserver`,
`workflow`).
...
- **Resource**: The object the event relates to (MCPServer (or Workflow))
...
In follow mode the resource/type filters (`--resource-type`, `--resource-name`,
`--namespace`, `--type`) apply to both the initial snapshot and the streamed
events. `--limit` bounds only the initial snapshot; the live stream is
unbounded. `--since` / `--until` apply only to the initial snapshot — a watch
only ever surfaces new events, so a time window does not constrain the stream.
...
- MCPServer tools become unavailable
