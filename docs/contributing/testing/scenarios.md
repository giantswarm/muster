...
| Authorization server restarted | `test_restart_mock_oauth_server: {server}` (`test_forget_oauth_registrations` for the loss alone) | A fresh process on the same port and issuer; under `mock_oauth_servers[].profile: pro` (or `forget_registrations_on_restart`) every RFC 7591 registration is gone with the old one -- an in-memory client store. The quirks of a named authorization server (GitHub, Dex, pro) are one `profile`, see "Authorization-server profiles" in oauth-testing.md |
