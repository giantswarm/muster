...
func TestWorkloadGrantsFromConfig_AppendsGroupGrants(t *testing.T) {
	...
	tests := []struct {
		name  string
		input config.WorkloadGroupGrantConfig
		check func(t *testing.T, g oauthserver.WorkloadGrant)
	}{
		{
			name: "groups only",
			input: config.WorkloadGroupGrantConfig{
				...
				Granted:   config.WorkloadGrantedIdentityConfig{Groups: []string{"giantswarm-ad:sre"}},
			},
			check: func(t *testing.T, g oauthserver.WorkloadGrant) {
				t.Helper()
				require.Equal(t, "https://kubernetes.default.svc", g.Issuer)
				require.Equal(t, "system:serviceaccount:kagent:sre-agent", g.Subject)
				require.Equal(t, []string{"mcp-prometheus"}, g.Audiences)
				require.Equal(t, []string{"giantswarm-ad:sre"}, g.Granted.Groups)
				require.Empty(t, g.Granted.Subject)
			},
		},
		{
			name: "groups and subject",
			input: config.WorkloadGroupGrantConfig{
				Issuer:    "https://kubernetes.default.svc",
				Subject:   "system:serviceaccount:kagent:sre-agent",
				Audiences: []string{"mcp-kubernetes"},
				Granted: config.WorkloadGrantedIdentityConfig{
			...
			check: func(t *testing.T, g oauthserver.WorkloadGrant) {
				t.Helper()
				require.Equal(t, []string{"agent:sre"}, g.Granted.Groups)
				require.Equal(t, "agent:sre", g.Granted.Subject)
			},
		},
		{
			name: "no granted identity",
			input: config.WorkloadGroupGrantConfig{
				Issuer:    "https://kubernetes.default.svc",
				Subject:   "system:serviceaccount:kagent:sre-agent",
				Audiences: []string{"mcp-kubernetes"},
			...
			check: func(t *testing.T, g oauthserver.WorkloadGrant) {
				t.Helper()
				require.Empty(t, g.Granted.Groups)
				require.Empty(t, g.Granted.Subject)
			},
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			grants := workloadGrantsFromConfig(nil, []config.WorkloadGroupGrantConfig{tc.input})
			require.Len(t, grants, 1)
			tc.check(t, grants[0])
		})
	}
