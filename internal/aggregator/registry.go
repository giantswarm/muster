...
// RegisterPendingAuth registers a server reachable but requiring authentication
// before its tools can be exposed. Sets no auth config; equivalent to
// RegisterPendingAuthWithConfig(name, url, toolPrefix, authInfo, nil).
...
// RegisterPendingAuthWithConfig registers a pending-auth server and records
// its auth configuration so SSO forwarding or RFC 8693 exchange can drive
// the session at login time. Flat wrapper around registerPendingAuthLocked
// for callers that don't need to declare a family.
func (r *ServerRegistry) RegisterPendingAuthWithConfig(name, url, toolPrefix string, authInfo *AuthInfo, authConfig *api.MCPServerAuth) error {
	return r.registerPendingAuthLocked(PendingAuthRegistration{
...
// RegisterPendingAuthFromRegistration registers a pending-auth server from a
// fully-formed registration including any declared spec.family. Callers that
// need family-aware pending-auth wiring (reconciler-driven federation) use
// this form; flat wrappers above are kept for tests and adapters that don't.
func (r *ServerRegistry) RegisterPendingAuthFromRegistration(registration PendingAuthRegistration) error {
	return r.registerPendingAuthLocked(registration)
}

// registerPendingAuthLocked is the underlying implementation used by both the
// flat wrappers and the registration-struct entry point. Per ADR-008 no
// synthetic auth tools are created — users authenticate via core_auth_login.
func (r *ServerRegistry) registerPendingAuthLocked(registration PendingAuthRegistration) error {
	r.mu.Lock()
	defer r.mu.Unlock()
