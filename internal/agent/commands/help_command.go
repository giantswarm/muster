...
func (h *HelpCommand) showGeneralHelp() {
	...
	h.output.OutputLine("  call <tool> [params...]      - Execute a tool (supports key=value or JSON)")
	...
	h.output.OutputLine("  context [list|use <name>]    - List or switch muster contexts")
	h.output.OutputLine("  exit, quit                   - Exit the REPL")
	h.output.OutputLine("")
	h.output.OutputLine("Keyboard shortcuts:")
	h.output.OutputLine("  TAB                          - Auto-complete commands, tool names, and parameters")
	...
	h.output.OutputLine("")
	h.output.OutputLine("Examples (call/prompt/workflow commands support both key=value and JSON syntax):")
	h.output.OutputLine("  call kubernetes_list resourceType=pod namespace=default")
	h.output.OutputLine("  call kubernetes_list {\"resourceType\": \"pod\", \"namespace\": \"default\"}")
	...
	h.output.OutputLine("")
	h.output.OutputLine("Tip: Type a command without arguments to see available parameters")
}
