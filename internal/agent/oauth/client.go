...
func (c *Client) RefreshTokenIfNeeded(ctx context.Context, serverURL string) (bool, error) {
	storedToken := c.tokenStore.GetTokenIncludingExpiring(serverURL)
	if storedToken == nil {
		return false, ErrAuthRequired
	}

	// Check if token needs refresh (expires within threshold)
	if !c.tokenNeedsRefresh(storedToken) {
		return false, nil
	}

	// Check if we have a refresh token
	if storedToken.RefreshToken == "" {
		slog.Debug("Token needs refresh but no refresh token available",
	...
	// Perform the refresh
	newToken, err := c.doRefreshToken(ctx, storedToken)
	if err != nil {
		return false, err
	}

	// Store the new token
	if err := c.tokenStore.StoreToken(serverURL, storedToken.IssuerURL, newToken); err != nil {
		slog.Debug("Failed to store refreshed token",
			"server_url", serverURL,
			"error", err.Error(),
		)
...
func (c *Client) doRefreshToken(ctx context.Context, storedToken *StoredToken) (*oauth2.Token, error) {
	if storedToken.RefreshToken == "" {
		return nil, errors.New("no refresh token available")
	}

	slog.Debug("Refreshing OAuth token",
		"server_url", storedToken.ServerURL,
		"issuer_url", storedToken.IssuerURL,
	)

	// Discover OAuth metadata to get the token endpoint
	metadata, err := c.oauthClient.DiscoverMetadata(ctx, storedToken.IssuerURL)
	if err != nil {
		return nil, fmt.Errorf("failed to discover OAuth metadata: %w", err)
	}

	// Use the shared OAuth client to refresh the token
	newToken, err := c.oauthClient.RefreshToken(ctx, metadata.TokenEndpoint, storedToken.RefreshToken, DefaultAgentClientID)
	if err != nil {
	...
	// Convert pkg/oauth.Token to oauth2.Token
	...
	// Preserve refresh token if not returned (some providers don't rotate refresh tokens)
