...
type MCPServerAuth struct {
	...
	// RFC 8693 broker.
	//
	// Use LocalMint when the backend must authorize an agent acting on behalf of a
	// human (OBO) and no shared remote IdP can issue a token with the backend's
	// audience. Requires muster's OAuth server to run in
