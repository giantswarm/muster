...
Note: The aggregator server must be running (use 'muster serve') before using these commands.`,
	...
	FParseErrWhitelist: cobra.FParseErrWhitelist{
		UnknownFlags: true, // Allow unknown flags for MCPServer parameters
	},
	RunE: runCreate,
}
...
func processMCPServerFlag(args map[string]interface{}, flagName, flagValue string, hasValue bool) {
	...
	case "forward-identity", "forwardIdentity":
		getOrCreateObjectMap(args, "auth")["forwardIdentity"] = !hasValue || flagValue == stringTrue
...
func runCreate(cmd *cobra.Command, args []string) error {
	...
	// Handle other resource types (workflow)
	if len(args) < 2 {
	...
	if !exists {
		return fmt.Errorf("unknown resource type '%s'. Available types: workflow, mcpserver", resourceType)
	}
