...
- `muster events --follow` now works. It previously blocked forever waiting on an MCP notification channel the server never wrote to (server-side streaming was an unimplemented stub). Follow is now implemented client-side by polling the `core_events` tool, printing existing events once and then streaming new ones with de-duplication; the dead server-side streaming goroutine and notification stub were removed.
