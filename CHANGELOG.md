...
- **BREAKING: Dex cross-client audiences need an allowlist.** `oauth.server.dex.allowedAudiences` lists the audiences muster may request from Dex. An MCPServer's `spec.auth.requiredAudiences` value is requested only when it is listed there, and an empty list (the default) denies every cross-client audience. Requesting an audience widens every user's forwarded ID token to that peer, and every `forwardToken` backend receives that token, so the set belongs to the operator who owns the Dex clients rather than to whoever can create an MCPServer. **Deploy note:** set the value in the same rollout, or a `forwardToken` backend that needs an audience stops receiving it. On Giant Swarm installations the value is `dex-k8s-authenticator`. Each denied audience is logged once with the key to set.

### Fixed

- Dex cross-client audience scopes are resolved on every authorization request instead of once when the OAuth server is built. An MCPServer with `forwardToken: true` that registers after muster starts now reaches the next login, so the forwarded ID token carries the audience its backend expects. Existing sessions are not repaired: affected users must log in again once. The audience set a login requests is logged whenever it changes.
