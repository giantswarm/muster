...
- Bump architect-orb pin from `8.1.0` to `8.2.2`. Picks up the `sync-china-registry` POSIX-shell fix (v8.1.0 used bash-only syntax in `image-login-to-registries` which broke on the alpine/BusyBox executor — every `split-china-push: true` Aliyun mirror invocation was failing on its first step) and `trimpath: true` default for the `go-build` job (in-image binary is now byte-reproducible across hosts).
