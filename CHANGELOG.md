...
- Token-exchange (oidc-exchange) backends no longer get stuck in `Auth Required` once their exchanged token expires. The persistent aggregator connection carries mcp-go's continuous listener, which outlives the exchanged token's fixed lifetime (Dex default 30 minutes) and never went through the tool-call refresh path, so an idle backend's token expired and the listener looped it indefinitely (`401 Token validation failed` / `403 invalid session`) while the MCPServer sat in `Auth Required`. The connection now re-exchanges a fresh token before expiry, and evicts itself only when the subject token can no longer be refreshed.
