...
- The admin UI listener is now protected by the OAuth server's token validation when `oauth.server` is enabled. With OAuth disabled, the listener refuses to bind a non-loopback `aggregator.admin.bindAddress` and logs that it is running unauthenticated on loopback.
