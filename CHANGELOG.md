...
- Unused `app.Config.Silent` field. The `--silent` CLI flag stays — it still swaps the logger's writer to `io.Discard` in `cmd/serve.go` — but the field was set by `NewConfig` and never read. `NewConfig` drops the `silent` parameter accordingly.

### Changed

- Logging bootstrap now lives in `cmd/serve.go`. The serve command calls `logging.Init` once at startup, defers the `Shutdown`, then constructs the application. `NewApplication` no longer touches the logger; CLI commands that need logs initialise via `logging.InitForCLI` themselves.
