...
- `enableJWTMode: true` now actually issues JWT access tokens. The OAuth server previously hard-failed on startup when no signing key was configured, silently falling back to opaque tokens. Set `muster.oauth.server.jwtSigningKey` (PEM-encoded EC P-256 or RSA key) or use `existingSecret` with key `jwt-signing-key`; `helm template` fails if neither is provided when `enableJWTMode: true`.
