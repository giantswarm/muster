...
- Outbound OAuth flows now send the [RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html) `resource` parameter on both the authorization request and the token request, as MCP 2026-07-28 requires. The value is the canonical URI the target declares in its RFC 9728 metadata, and the canonical form of the configured URL only when that metadata omits the field or `spec.auth.authorizationServer` opts out of discovery. This holds for backends and for the `muster agent` login, which reads the `resource` field of the muster server's own protected resource metadata. A token muster obtains for one backend is no longer accepted at another backend that trusts the same issuer, provided the authorization server honors the parameter. Authorization servers that reject an unknown `resource` will fail the login; the spec requires sending it regardless.
- Authorization server metadata is now rejected when its `issuer` does not identify the server the document was fetched from ([RFC 8414](https://www.rfc-editor.org/rfc/rfc8414.html) §3.3, trailing slash ignored). A misconfigured or hostile authorization server can no longer name a different issuer and have muster compare RFC 9207 `iss` values against that claim. A document without an `issuer` is rejected as well.
...
- OAuth callbacks are now validated per [RFC 9207](https://www.rfc-editor.org/rfc/rfc9207.html) before anything else on the response is acted on. A present `iss` must equal the issuer of the authorization server the flow was sent to (simple string comparison, no normalization), and an absent `iss` is refused when that server advertises `authorization_response_iss_parameter_supported`. A response that fails the check is rejected whole: its `error` and `error_description` are neither acted on nor displayed. This also reorders the proxy callback: an error response that carries no valid state now renders "missing required parameters" instead of the OAuth denial page, because muster cannot attribute it to a flow.
