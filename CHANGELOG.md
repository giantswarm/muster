...
- **Breaking:** muster CRDs (`mcpservers.muster.giantswarm.io`, `workflows.muster.giantswarm.io`) ship in a new sibling chart `muster-crds`. `helm/muster/templates/crds.yaml` is removed; `crds.install` in `helm/muster/values.yaml` defaults to `false` and is now a no-op kept for value compatibility (removed in chart v0.2.0). Install / upgrade `muster-crds` before `muster`. See [`helm/muster-crds/UPGRADE.md`](helm/muster-crds/UPGRADE.md) for the schema-change procedure.
- `controller-gen` writes CRDs to `helm/muster-crds/crds/`; `make generate-crds` and the CI drift check follow the new path.
- `helm/muster/templates/ciliumnetworkpolicy.yaml` adds an unconditional egress allowance to `app.kubernetes.io/name=agentgateway:8080` in the release namespace so muster's aggregator can dial the agentgateway data-plane on the upstream-proxy path. No-op when the data-plane is not deployed (selector matches zero endpoints).
- `ciliumNetworkPolicy.worldEgressPorts` value lets operators extend the policy's `world` egress beyond the default `80/443`. MCPServers running on non-standard ports (e.g. `5000`, `8443`) silently 503'd before this — the policy is default-deny.
- New `extraEnv` and `extraArgs` values on the muster Deployment for pushing env vars / CLI flags without forking the chart (the `MUSTER_AGW_UPSTREAM_URL` pre-helper workaround needed `kubectl set env`).
- `livenessProbe` and `readinessProbe` are now full-shape values instead of hard-coded `httpGet /health` — operators on muster binaries that don't bind `/health` early (or at all) can swap to a TCP socket probe by setting `livenessProbe.httpGet: null` + `tcpSocket: {port: http}` in their values override. The default still targets `/health`.
- `NOTES.txt` shows a canonical `gatewayAPI.httpRoute.parentRefs` / `hostnames` example pointing at `envoy-gateway-system/giantswarm-default` when the fail-guard would block install — surface the shape operators previously dug out of `values.yaml`.
- `helm/muster/templates/deployment.yaml` honours a new `revisionHistoryLimit` value (default `3`) so frequent rollouts don't leak zero-replica ReplicaSets.
- `helm/muster/values.yaml` declares `resources.{requests,limits}.ephemeral-storage` on the muster container (`50Mi` request, `100Mi` limit). The container mounts `/tmp` as `emptyDir`; without an explicit ephemeral-storage limit, Kyverno's audit policy on Giant Swarm workload clusters warns and strict-enforce clusters reject the pod.
