...
- `enableJWTMode: true` now actually issues JWT access tokens. Previously the OAuth server's `Validate()` hard-failed on startup when `AccessTokenSigningKey` was absent, causing muster to silently fall back to opaque tokens regardless of the configured mode. The Helm chart now supports a `muster.oauth.server.jwtSigningKey` value that writes the PEM-encoded private key into the oauth `Secret`; the chart emits the `jwtSigningKeyFile` ConfigMap key only when a key or `existingSecret` is provided, and fails with an explicit `helm template` error when `enableJWTMode: true` and neither is set. Supported algorithms: EC P-256 (ES256) and RSA ≥ 2048-bit (RS256); `kid` is the RFC 7638 JWK thumbprint.
...
- `oauth.server.machinePrincipals`: maps K8s ServiceAccount `sub` claim values to synthetic user identities. During RFC 8693 token exchange the broker looks up the validated subject in this map and injects the configured `email` and `groups` into the issued muster JWT, so downstream MCPs can impersonate the synthetic user without a separate IdP lookup. Helm: `muster.oauth.server.machinePrincipals` (object, keyed by SA `sub` string).
