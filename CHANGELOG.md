...
- The admin UI session-detail JSON endpoint no longer serializes raw signed bearer tokens. Stored JWTs are exposed only as their decoded header and payload (`decodedTokens`); the compact token and its signature never leave the server.
- The admin UI listener is now protected by the OAuth server's token validation when `oauth.server` is enabled. With OAuth disabled, a non-loopback `aggregator.admin.bindAddress` is refused and only a loopback bind is served (unauthenticated). When OAuth is enabled on a transport with no HTTP listener (`stdio`), the admin UI is not started rather than served unprotected.
