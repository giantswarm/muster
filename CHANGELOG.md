...
- **BREAKING: `oauth.server.dex.requestableAudiences` gates the audiences muster requests from Dex.** An MCPServer's `spec.auth.requiredAudiences` value is requested only when it is listed there. An empty list, the default, denies every cross-client audience, and each denied audience is logged once with the key to set. **Deploy note:** set the value in the same rollout, or a `forwardToken` backend that needs an audience stops receiving it. On Giant Swarm installations the value is `dex-k8s-authenticator`.

### Fixed

- Dex cross-client audience scopes are resolved on every authorization request instead of once when the OAuth server is built. An MCPServer with `forwardToken: true` that registers after muster starts now reaches the next login, so the forwarded ID token carries the audience its backend expects. Existing sessions are not repaired: affected users must log in again once. The resolved set is cached for 10 seconds, so a login costs no MCPServer list call in that window. The audience set a login requests is logged whenever it changes.
