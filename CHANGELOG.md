...
- Outbound OAuth flows now send the [RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html) `resource` parameter on both the authorization request and the token request, as MCP 2026-07-28 requires. The value is the canonical URI the backend declares in its RFC 9728 metadata, or the canonical form of the configured server URL when the metadata omits it or `spec.auth.authorizationServer` opts out of discovery. The `muster agent` login flow sends the canonical form of the muster server URL. A token muster obtains for one backend is no longer accepted at another backend that trusts the same issuer, provided the authorization server honors the parameter. Authorization servers that reject an unknown `resource` will fail the login; the spec requires sending it regardless.
