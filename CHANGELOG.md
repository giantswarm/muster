...
- MCPServers using RFC 8693 token exchange no longer restart on every reconcile pass. `ConfigurationChanged` compared `Auth` with `reflect.DeepEqual`, but `TokenExchangeConfig.ClientID`/`ClientSecret` are resolved from a Secret at runtime and stored on the running definition while being absent from a definition rebuilt from the CR (they are tagged `json:"-" yaml:"-"`). Once credentials loaded, the comparison always reported a difference, so the reconciler tore down and reconnected the MCPServer every ~10-15s; a tool call landing in the restart/reauth window failed with `authentication required` / `user not authenticated to server`. The auth comparison now ignores these runtime-only credential fields while still detecting every spec change. ([#37060](https://github.com/giantswarm/giantswarm/issues/37060))
