...
- OpenTelemetry tracing for every MCP tool call. The aggregator's mcp-go middleware opens a `tool.<name>` span at the meta-tool layer, and `CallToolInternal` opens an inner span carrying the real workload tool name (`x_kubernetes_*`, `workflow_*`, `service_*`, …) so Tempo trace trees pivot on the actual tool. W3C TraceContext + Baggage propagators are installed unconditionally so inbound `traceparent` headers are honoured even when no exporter is configured. Tracer-provider lifecycle handled by [`mcp-toolkit v0.1.0`](https://github.com/giantswarm/mcp-toolkit/releases/tag/v0.1.0)'s `tracing` package.
- OpenTelemetry metrics for every MCP tool call: `muster.tool_calls` (counter, exports as `muster_tool_calls_total`) and `muster.tool_call.duration` (histogram, exports as `muster_tool_call_duration_seconds`), each with `tool` and `outcome` attributes (`ok` / `error` / `error_result`). Meter-provider lifecycle handled in `internal/aggregator/instrument` until `mcp-toolkit` ships a meter helper.
