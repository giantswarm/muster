...
- `docs/operations/installation.md` covers the muster + agentgateway data plane: filesystem-mode subprocess auto-spawn (binary resolver precedence, combined `<configPath>/agentgateway/agentgateway.yaml`, listener defaults), cluster-mode per-MCPServer CRD emission (`AgentgatewayBackend` + `HTTPRoute` + `AgentgatewayPolicy`), `spec.suspended` pause/resume, and the deprecation note on `core_service_list` / `core_service_status`.
- Helm chart `ClusterRole` grants the muster ServiceAccount full CRUD on `agentgateway.dev/v1alpha1` `AgentgatewayBackend` + `AgentgatewayPolicy` and `gateway.networking.k8s.io/v1` `HTTPRoute`. Without this the cluster-mode reconciler cannot emit the agentgateway config stack on Kubernetes clusters that follow the chart's RBAC.
- BDD scenario coverage for the muster-in-front-of-agentgateway pivot: `mcpserver-required-audiences-config.yaml` (CRUD + multi-value + update for `spec.auth.requiredAudiences`); `mcpserver-token-exchange-config.yaml` (RFC 8693 standalone — non-Teleport — CRUD + mutual-exclusivity rejection); `mcpserver-reconnect.yaml` (`core_mcpserver_reconnect` force-reconnect with tool-call survival); `agentgateway-subprocess-stdio-target.yaml` (filesystem-mode stdio MCPServer spawned by agentgateway via `mcp.targets[].stdio`). `mcpserver-authorization-server-override.yaml` (issue #599) extended with an update assertion so the override survives `core_mcpserver_update`.
...
- `api.AggregatorHandler.UpstreamServerStateForSession(ctx, name)` — returns `UpstreamServerConnected` for OAuth servers whose global registry stays in pending-auth once the calling session has a live pooled connection. `core_service_list` and `core_service_status` use it so post-SSO state is reflected.
- Filesystem mode spawns `agentgateway` as a child process against a single combined `<configPath>/agentgateway/agentgateway.yaml` and stops it on shutdown. The yaml emitter (`internal/reconciler/agentgateway/yaml`) is stateful: all MCPServers share one bind and one listener (default name `muster`, override via `WithListenerName`); per-MCPServer routes accumulate into a sorted list and the combined file is rewritten atomically on every Apply/Delete. agentgateway v1.2.1's `-f` flag takes one file (no `--config-dir`, no multi-`-f`, no glob), so per-MCPServer files would not work; an empty Applier emits `binds: []` so agentgateway can start before any MCPServer reconciles. `MCPServerReconciler.WithDisableLocalSpawn(true)` is set in filesystem mode so the orchestrator does not race agentgateway over the same stdio MCP children. `subprocess.Manager.PID()` exposes the live agentgateway pid for diagnostics. agentgateway v1.2.1 pinned via `internal/agentgateway/binary.Resolve`.
...
- MCPServer reconciler in cluster mode emits the full agentgateway config stack — `AgentgatewayBackend`, `HTTPRoute`, `AgentgatewayPolicy` (all `agentgateway.dev/v1alpha1`, CRD Go types imported from upstream `github.com/agentgateway/agentgateway/controller/api/v1alpha1/agentgateway`) — automatically per MCPServer with OwnerReferences on the MCPServer so deletion cascades. Manual per-backend `AgentgatewayBackend` + `HTTPRoute` YAML is no longer required. Stdio MCPServers in cluster mode surface a `NotSupportedInCluster` status condition (per-MCPServer pod isolation; deferred). The aggregator service no longer routes through the orchestrator service registry; `internal/app` starts it directly between the StateChangeBridge and the agentgateway subprocess and stops it before the orchestrator on shutdown.

### Added

- `internal/agentgateway/version` package holds the single agentgateway version pin (`Pinned`, `Tag`) mirrored from the require entry for `github.com/agentgateway/agentgateway` in `go.mod`. `binary.PinnedVersion` and the yaml-applier `SchemaURL` derive from it; the comment in `local_types.go` no longer carries a version literal. New Makefile targets: `make refresh-pin` regenerates `internal/agentgateway/version/pin.go` from `go list -m`, `make refresh-checksums` regenerates `internal/agentgateway/binary/checksums.go` from upstream `.sha256` files, `make refresh-agentgateway` runs both. `make verify-pin` / `make verify-checksums` / `make verify-agentgateway` catch drift; the offline `verify-pin` is wired into `.pre-commit-config.yaml` so Renovate-driven `go.mod` bumps that forget the regen fail locally. The checksum map keys drop the version segment (the map is implicitly for the current `Pinned`).
...
- `internal/reconciler/state_change_bridge.go` deleted. It only reacted to `serviceType == "MCPServer"` events; after PR 11 there are no MCPServer services in the orchestrator's registry, so the bridge subscribed to an event stream that never fired.
- API surface trimmed: `api.OrchestratorAPI`, `api.NewOrchestratorAPI`, `api.ServiceRegistryHandler`, `api.RegisterServiceRegistry`, `api.GetServiceRegistry`, `api.ServiceInfo`, `api.ConfigurableService`, `api.ServiceStateChangedEvent`, `api.ServiceListResponse`, `api.StateUpdater`, `api.UpdateMCPServerState`, and `ServiceManagerHandler.SubscribeToStateChanges` all removed. `aggregator.NewAggregatorManager` no longer takes orchestrator/registry args; `Services.{Orchestrator, OrchestratorAPI, StateChangeBridge}` fields gone. The `notifyMCPServerConnected` call sites in `connection_helper.go` (which fed `api.UpdateMCPServerState` against the empty registry) are removed; SSO completion no longer pretends to publish a state update no consumer reads.
- Post-pivot dead code in the same sweep: `internal/services/mcpserver/` (the `BaseService` wrapper used to spawn MCPServer subservices through the orchestrator), `internal/mcpserver/client_sse.go`, `internal/mcpserver/client_stdio.go`, and `internal/mcpserver/client_factory.go` (`NewMCPClientFromType` + `MCPClientConfig` — the spec-type dispatch; muster now exclusively dials streamable-http through the upstream proxy). `NewStreamableHTTPClientWithHTTPClient` likewise dropped — only the factory consumed it. `subprocess.Manager.Reload` and `ErrNotRunning` removed: agentgateway watches its config file natively and no production caller invoked SIGHUP. `yaml.WithListenerName` removed (tests-only override; `DefaultListenerName = "muster"` is the single source of truth). `agentgatewayDefaultAdminPort` / `agentgatewayDefaultStatsPort` constants in `internal/app/services.go` dropped (never read). Stale narrative comments referencing the deleted orchestrator-driven service lifecycle trimmed from `internal/app/services.go`.
...
- The agentgateway translator's `Applier` and `Deleter` ports collapse into one `Applier` interface with a `Delete` method; the cluster-mode emitter's `Delete` is a no-op (cleanup cascades via OwnerReferences). `NewMCPServerReconcilerFilesystem` and `NewMCPServerReconcilerCluster` collapse into a single `NewMCPServerReconciler` taking an `ApplierFunc` closure for per-Reconcile applier construction. `resolveOwnerRef` lifts to an exported free function `reconciler.OwnerRefFor` so the cluster-mode closure can build the per-MCPServer ownerRef without holding a reconciler reference. No external behaviour change.
- Muster's aggregator now dials agentgateway at `<UpstreamProxy>/mcp/<server-name>` (streamable-http, regardless of `spec.type`) for every external MCPServer. `AggregatorConfig.UpstreamProxy` is required; cluster mode reads `MUSTER_AGW_UPSTREAM_URL` (Helm value `muster.agentgateway.upstreamURL`), filesystem mode defaults to `http://localhost:8080` (the agentgateway subprocess listener). Federation registration is reconciler-driven through `aggregator.RegisterUpstream` / `DeregisterUpstream` on every MCPServer reconcile / delete; the orchestrator no longer owns MCPServer lifecycle. `orchestrator.Config.DisableMCPServerAutoStart`, `MCPServerReconciler.WithDisableLocalSpawn`, the orchestrator's `processAutoStartMCPServers` / `createMCPServerService` / `handleAuthRequiredServer` / `retryFailedMCPServers` / `waitForMCPServerRegistration`, the aggregator manager's `registerHealthyMCPServers` / `EventHandler` / `retryFailedRegistrations`, and the `internal/services/aggregator` `BaseService` wrapper are removed; `runOrchestrator` constructs `aggregator.AggregatorManager` directly and supervises its lifecycle. `MCPServer.status.consecutiveFailures` / `lastAttempt` / `nextRetryAfter` are no longer updated — the per-service retry state machine is gone; the fields remain on the CRD for forward compatibility. Token exchange, server-family grouping, and ADR-006 session-scoped tool filtering continue to be handled by muster; agentgateway is transparent middleware for tracing, audit, and metrics.
...
- The aggregator's per-route dial path now runs a synchronous `:15021/healthz/ready` readiness handshake (filesystem mode) before `client.Initialize`, with a tight 4×100ms `ECONNREFUSED` retry to absorb agentgateway's file-watch reload bind race. Cluster mode skips the readiness probe (agentgateway runs out-of-band).
