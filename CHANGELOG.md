...
- `MCPServer.spec.suspended` — boolean. When true, the reconciler deregisters the upstream and skips subsequent `RegisterUpstream` pulses. `core_service_stop` / `core_service_start` mutate this field (replacing the previous in-memory `userStopped` set on the aggregator), so operator intent survives muster restarts.
