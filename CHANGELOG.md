...
- New `MCPServer.spec.transport` section selects how muster reaches the MCP endpoint and any transport-private endpoints (e.g. Dex `/token`). Carries no identity. Today the discriminator `spec.transport.type` accepts only `teleport`; future transports (e.g. wireguard) extend it additively. Under `spec.transport.teleport`, every Teleport-routed CR states its app names AND identity-secret refs verbatim — `mcp.appName` + `mcp.identitySecretRef` (required) name the Teleport app and local Secret used for the MCP HTTP call, and `dex.appName` + `dex.identitySecretRef` (optional) name the same for the Dex token-exchange path. No naming-convention derivation, no implicit cluster symbol; the CR is self-describing and grep-able across the GitOps tree, and cluster-admins can name secrets to match their existing tbot-output topology. Customer Muster (in-VPN) omits this field — direct-HTTPS to `spec.url` is preserved. Four CRD-level CEL rules guard the field: `transport.teleport` is required when `transport.type=teleport`, may not be set otherwise, `transport` is forbidden when `spec.type=stdio`, and `transport.teleport.dex` is required when `spec.auth.type=oauth` AND `spec.auth.tokenExchange.enabled=true` (the token-exchange path needs a routable Dex transport).
