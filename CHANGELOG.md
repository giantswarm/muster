...
- MCPServer reconciler now emits the full agentgateway config stack — `AgentgatewayBackend`, `HTTPRoute`, `AgentgatewayPolicy` (all `agentgateway.dev/v1alpha1`, CRD Go types imported from upstream `github.com/agentgateway/agentgateway/controller/api/v1alpha1/agentgateway`) — automatically per MCPServer in cluster mode, with OwnerReferences on the MCPServer so deletion cascades. Manual per-backend `AgentgatewayBackend` + `HTTPRoute` YAML is no longer required. Stdio MCPServers in cluster mode surface a `NotSupportedInCluster` status condition (per-MCPServer pod isolation; deferred). Both Appliers also emit a baseline `/mcp/muster` route pointing at muster's own aggregator (`yaml.WithMusterBackend` for filesystem mode; `k8s.WithMusterBackend` + `EnsureMusterBaseline` for cluster mode), so agentgateway federates muster's intrinsic tools (workflows, meta-tools, capabilities) alongside external MCPServers behind one client-facing endpoint. Cluster baseline objects carry `app.kubernetes.io/managed-by=muster-operator` for Helm-uninstall cleanup. The aggregator service no longer routes through the orchestrator service registry; it is started directly by `internal/app` between the StateChangeBridge and the ReconcileManager and stopped before the agentgateway subprocess on shutdown. New Helm values / env vars: `MUSTER_SERVICE_NAME` (default `muster`), `MUSTER_SERVICE_NAMESPACE` (default the operator's namespace), `MUSTER_SERVICE_PORT` (default `8090`).
